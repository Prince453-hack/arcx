...
interface EditorStore {
  ...
  closeTab: (projectId: Id<"project">, fileId: Id<"files">) => void;
...
export const useEditorStore = create<EditorStore>()((set, get) => ({
  ...
  getTabState: (projectId) => {
    return get().tabs.get(projectId) ?? defaultState;
  },
  openFile: (projectId, fileId, { pinned }) => {
    const tabs = new Map(get().tabs);
    const state = tabs.get(projectId) ?? defaultState;
  ...
  closeAllTabs: (projectId) => {
    const tabs = new Map(get().tabs);
    tabs.set(projectId, defaultState);
    set({ tabs });
  },
